...
class SeDRAM : public IDRAM, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAM, SeDRAM, "SeDRAM", "SeDRAM Device Model")

  public:
    inline static const std::map<std::string, Organization> org_presets = {
      //   name         density   DQ   Ch Ra Bg Ba   Ro     Co
      {"mySeDRAM_8Gb_x8",   {8<<10,   8,  {1, 1, 4, 4, 1<<16, 1<<10}}},
    };
    ...
    inline static const std::map<std::string, std::vector<int>> timing_presets = {
      ...
      {"mySeDRAM_ocl",  {3200,   4,  12,  10,   10,   29,   45,   16,   9,   12,   4,    6,   -1,   -1,    3,    9,   -1,  -1,  -1,   2,    625} },
    };
  ...
  private:
    ...
    void set_timing_vals() {
      ...
      if (dq_id != -1 && rate_id != -1) {
        ...
        std::cout << "nRRDS value: " << m_timing_vals("nRRDS") << " " << std:: endl;
        m_timing_vals("nRRDL") = nRRDL_TABLE[dq_id][rate_id];
        std::cout << "nRRDL value: " << m_timing_vals("nRRDL") << " " << std::endl;
        m_timing_vals("nFAW")  = nFAW_TABLE [dq_id][rate_id];
        std::cout << "nFAW value: " << m_timing_vals("nFAW") << " " << std::endl;
      }
      ...
      std::cout << "nRFC value: " << m_timing_vals("nRFC") << " " << std::endl;
      m_timing_vals("nREFI") = JEDEC_rounding(tREFI_BASE, tCK_ps);
      std::cout << "nREFI value: " << m_timing_vals("nREFI") << " " << std::endl;
